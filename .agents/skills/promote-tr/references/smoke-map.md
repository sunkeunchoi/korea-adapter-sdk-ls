...
| `CSPAT00701` | `live-smoke-order-chain` | `order_chained_smoke` | gate 2 — chained submit→modify→cancel; ORDER-CAPABLE paper account + `LS_ORDER_SMOKE=1`; places REAL paper orders; modify-link failure → gate 2 PENDING (gate 1 stands) | implemented-only | paper cash-equity order MODIFY — gate 2 evidence (plan -005) |
| `CSPAT00801` | `live-smoke-order-chain` | `order_chained_smoke` | gate 2 — chained submit→modify→cancel (the PRIMARY teardown); ORDER-CAPABLE paper account + `LS_ORDER_SMOKE=1`; cancel-link failure → gate 2 PENDING + paper-reset fallback | implemented-only | paper cash-equity order CANCEL — gate 2 evidence + teardown (plan -005) |
