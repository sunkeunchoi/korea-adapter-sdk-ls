...
//! record → adoption-state reporting, plus the Enforced fail-closed missing-snapshot
//! contract (KTD8). Enforced is the sole surviving posture after the #189 weekday retirement.
...
fn composition_root_resolves_loads_injects_and_reports_adoption() {
    ...
    let loaded = resolve_and_load(Some(&path), as_of(), CalendarAdoption::Enforced);
    assert!(loaded.is_available(), "a valid snapshot must inject");
    assert!(loaded.calendar().is_some());

    // Startup diagnostic + adoption-state reporting: Enforced over a healthy trading day
    // reports the calendar as authoritative.
    ...
    assert!(line.contains("adoption=enforced"), "{line}");
...
#[test]
fn enforced_missing_snapshot_resolves_typed_and_fails_closed() {
    // A path is configured, but NO snapshot exists there.
    let dir = tempfile::tempdir().unwrap();
    let missing = dir.path().join("does-not-exist.json");

    // resolve_and_load returns a typed Unavailable, never panics — even under Enforced,
    // where the consumer then fails closed.
    let loaded = resolve_and_load(Some(&missing), as_of(), CalendarAdoption::Enforced);
    match &loaded {
    ...
    // The startup record reports Enforced failing closed on the unavailable calendar.
    let rec = build_startup_record(
        "smoke",
        CalendarAdoption::Enforced,
        &loaded,
    ...
    let line = rec.render_line();
    assert!(line.contains("action=enforced-fail-closed"), "{line}");
    assert!(line.contains("snapshot=unavailable"), "{line}");
}

#[test]
fn no_snapshot_configured_is_not_configured_and_fails_closed() {
    let loaded = resolve_and_load(None, as_of(), CalendarAdoption::Enforced);
