...
| `report tiers` | The Turn-N per-tier trade-count report + power pre-check (plan 2026-07-10-003): joins exit envelopes to tagged universe-accept envelopes on (symbol, session), buckets by the pre-registered four-cell stratum, and reports counts against the ≥30-in-≥2-tiers floor → GREEN/RED. Also emits the per-tier opening-gap-% distribution from catalog daily bars (a red verdict's diagnosability: thin tier vs miscalibrated `gap_min_pct`). Asserts the KTD2 hash handshake — ingest pin ↔ run manifest ↔ artifact on disk — and fails on any mismatch. Counts only; never reads `performance.json` (the staging guard). | `LS_REPORT_RUN` (default: latest finalized), `LS_REPORT_METADATA` (artifact-path override; default the ingest pin's path) |
