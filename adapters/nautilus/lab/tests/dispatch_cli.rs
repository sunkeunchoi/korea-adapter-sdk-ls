...
fn bin_head_prints_the_code_hash_and_is_read_only() {
    ...
    assert!(stdout.contains("7571abef"), "frames the check against the documented v35 head: {stdout}");
    assert!(stdout.contains("NOT a v35 confirmation"), "the params-hash line is labeled version-invariant: {stdout}");
...
fn bin_rung_report_prints_the_head_hash_and_is_read_only() {
    ...
    assert!(stdout.contains("7571abef"), "frames against the documented v35 head: {stdout}");
