...
### Accumulate-forward
The Nautilus adapter's ingestion mode for history that cannot be backfilled: an idempotent, checkpoint-driven incremental pull that extends catalog coverage (whole-universe minute bars) from the last completed point to the present, re-snapshotting the instrument universe each run. It exists for two reasons — deep backfill is arithmetically unobtainable (a multi-year full-universe minute pull is ~10^6 paced requests, and server-side minute lookback is itself capped), and the universe reads (t8430/t9945) return only currently-listed symbols, so periodic re-snapshots are the only bound on survivorship bias; both coverage and the universe accrue from adoption day. The mode is safe to invoke at any time (no-ops when current, respects the live-session mutual-exclusion lock); scheduling lives outside the adapter as an operator cron recipe, never a daemon.
