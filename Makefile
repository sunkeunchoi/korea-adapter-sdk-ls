...
live-smoke:
	$(call run_smoke,market_session_price::live_smoke_default)
...
## PAPER-RESET: flatten the domestic paper book — cancel every resting order and close
## every long stock position (marketable SELL), then positively confirm flat. Use this
## when the order smokes (`make live-smoke-order` / `live-smoke-order-chain`) leave a
## resting order or a filled position that teardown could not unwind, instead of resetting
## the paper book by hand. It places REAL paper orders (marketable sells).
##
## SAME fail-closed guards as live-smoke-order-chain (the smoke refuses unless ALL hold):
##   - LS_TRADING_ENV=paper + LS_ORDER_SMOKE=1 (the standing double opt-in), AND
##   - NO CI/no-TTY marker (run in an attended PTY), AND
##   - a FRESH per-wave human nonce within TTL — mint it each wave:
##       export LS_ORDER_SMOKE_NONCE=$(date +%s)
##     (a static/reused nonce is rejected; do NOT put the nonce in .env.domestic — this
##      recipe sources the lane file and a stale value there would clobber it.)
## It cancels resting orders (t0425/CSPAT00801), closes sellable longs at the daily floor
## (t0424/t1102/CSPAT00601 SELL), then re-reads to confirm flat, bounded-retry. A holding
## that cannot be priced or sold now (degenerate band / T+2 unsettled) is reported as
## `flat=not-yet remaining=[...]` — the run still passes (it is a best-effort reset, not a
## certification). MbrNo defaults to NXT (override with LS_ORDER_SMOKE_MBRNO). Run in an
## attended PTY:
##   export LS_ORDER_SMOKE_NONCE=$(date +%s); make paper-reset
paper-reset:
	@lane="$(LS_SMOKE_LANE)"; [ -n "$$lane" ] || lane="domestic"; \
	lane_file=".env.$$lane"; \
	[ -f "$$lane_file" ] || { echo "FAIL: paper-reset: lane file $$lane_file missing (LS_SMOKE_LANE=$$lane); refusing to fall back to .env (wrong-account hazard)"; exit 1; }; \
	set -a; . "./$$lane_file"; set +a; \
	export LS_ORDER_SMOKE=1 LS_ORDER_SMOKE_TR=CSPAT00601; \
	out=$$(cargo test -p ls-sdk --test order_smoke -- --ignored --exact --nocapture chain::paper_reset 2>&1); \
	echo "$$out"; \
	echo "$$out" | grep -q "1 passed" || { echo "FAIL: paper-reset did not run (0 tests) or did not pass"; exit 1; }; \
	echo "$$out" | grep -q "PAPER-RESET canceled=" || { echo "FAIL: paper-reset produced no witness line"; exit 1; }

## R32 OVERNIGHT-HOLD PROBE (plan 2026-09-08-1215, queue `daily-probe-paper-overnight-hold`):
## the ONE order run that must NOT end flat. `r32-hold-place` buys ONE marketable
## CSPAT00601 position (LS_R32_SYMBOL, default 005930; LS_R32_QTY, default 1) at the
## band ceiling on the domestic paper lane, confirms the fill via t0425+t0424, prints a
## credential-free `R32-HOLD phase=place ...` witness (t0424 row + CSPAQ22200 deposit)
## and LEAVES THE POSITION IN THE BOOK. Same fail-closed guards as paper-reset
## (paper + LS_ORDER_SMOKE=1 + fresh LS_ORDER_SMOKE_NONCE + attended PTY). A buy that
## does not fill within the bounded wait is retry-canceled and the run hard-fails.
## NOTE: while the hold is in the book, the domestic lane's flat-asserting order
## smokes (`live-smoke-order-chain`) will report NOT flat by design — do not `paper-reset`
## until the verify phase has been recorded in the TURN-LOG.
##   export LS_ORDER_SMOKE_NONCE=$(date +%s); make r32-hold-place
r32-hold-place:
	@lane="$(LS_SMOKE_LANE)"; [ -n "$$lane" ] || lane="domestic"; \
	lane_file=".env.$$lane"; \
	[ -f "$$lane_file" ] || { echo "FAIL: r32-hold-place: lane file $$lane_file missing (LS_SMOKE_LANE=$$lane); refusing to fall back to .env (wrong-account hazard)"; exit 1; }; \
	set -a; . "./$$lane_file"; set +a; \
	export LS_ORDER_SMOKE=1 LS_ORDER_SMOKE_TR=CSPAT00601 LS_R32_PHASE=place; \
	out=$$(cargo test -p ls-sdk --test order_smoke -- --ignored --exact --nocapture chain::r32_overnight_hold 2>&1); \
	echo "$$out"; \
	echo "$$out" | grep -q "1 passed" || { echo "FAIL: r32-hold-place did not run (0 tests) or did not pass"; exit 1; }; \
	echo "$$out" | grep -q "R32-HOLD placed=confirmed" || { echo "FAIL: r32-hold-place produced no confirmed witness"; exit 1; }

## R32 verify phase — READ-ONLY (paper guard only, no order opt-in, nothing placed):
## the NEXT session's t0424 row + CSPAQ22200 deposit as `R32-HOLD phase=verify ...`.
## `held=no` is the design-invalidating outcome and is RECORDED, not failed.
##   make r32-hold-verify
r32-hold-verify:
	@lane="$(LS_SMOKE_LANE)"; [ -n "$$lane" ] || lane="domestic"; \
	lane_file=".env.$$lane"; \
	[ -f "$$lane_file" ] || { echo "FAIL: r32-hold-verify: lane file $$lane_file missing (LS_SMOKE_LANE=$$lane); refusing to fall back to .env (wrong-account hazard)"; exit 1; }; \
	set -a; . "./$$lane_file"; set +a; \
	export LS_R32_PHASE=verify; \
	out=$$(cargo test -p ls-sdk --test order_smoke -- --ignored --exact --nocapture chain::r32_overnight_hold 2>&1); \
	echo "$$out"; \
	echo "$$out" | grep -q "1 passed" || { echo "FAIL: r32-hold-verify did not run (0 tests) or did not pass"; exit 1; }; \
	echo "$$out" | grep -q "R32-HOLD phase=verify" || { echo "FAIL: r32-hold-verify produced no witness line"; exit 1; }
...
live-smoke-book:
	$(call run_smoke,market_session_price::live_smoke_book)
...
live-smoke-t0424:
	$(call run_smoke,account::live_smoke_t0424)

## Server-time smoke: read-only t0167 utility (always populated, closure-viable).
live-smoke-t0167:
	$(call run_smoke,market_session_charts::live_smoke_t0167)
...
live-smoke-clnaq00100:
	$(call run_smoke,account::live_smoke_clnaq00100)
...
live-smoke-cfoeq11100:
	$(call run_smoke,account::live_smoke_cfoeq11100)
...
live-smoke-cidbq01400:
	$(call run_smoke,account::live_smoke_cidbq01400)
...
live-smoke-cidbq03000:
	$(call run_smoke,account::live_smoke_cidbq03000)
...
live-smoke-cidbq05300:
	$(call run_smoke,account::live_smoke_cidbq05300)
...
live-smoke-cfobq10500:
	$(call run_smoke,account::live_smoke_cfobq10500)
...
live-smoke-ccenq90200:
	$(call run_smoke,account::live_smoke_ccenq90200)
...
live-smoke-cfoaq10100:
	$(call run_smoke,account::live_smoke_cfoaq10100)
...
live-smoke-ccenq10100:
	$(call run_smoke,account::live_smoke_ccenq10100)
...
live-smoke-ws:
	$(call run_smoke,realtime::live_smoke_ws)
...
live-smoke-k3:
	$(call run_smoke,realtime::live_smoke_k3)
...
live-smoke-ws-p1:
	$(call run_smoke,realtime::live_smoke_ws_p1)
...
live-smoke-ws-p2:
	$(call run_smoke,realtime::live_smoke_ws_p2)
...
live-smoke-ws-p3:
	$(call run_smoke,realtime::live_smoke_ws_p3)
...
## logging.
live-smoke-ws-p4:
	$(call run_smoke,realtime::live_smoke_ws_p4)
...
live-smoke-ws-negative:
	$(call run_smoke,realtime::live_smoke_ws_negative)

## t8425 (전체테마) smoke: paper guard -> OAuth token -> one all-themes read.
live-smoke-t8425:
	$(call run_smoke,market_session_masters::live_smoke_t8425)

## t8436 (주식종목조회) smoke: paper guard -> OAuth token -> one stock-list read.
live-smoke-t8436:
	$(call run_smoke,market_session_masters::live_smoke_t8436)

## t1531 (테마별종목) smoke: token -> t8425 theme -> one theme-constituents read.
live-smoke-t1531:
	$(call run_smoke,market_session_masters::live_smoke_t1531)

## t1537 (테마종목별시세) smoke: token -> t8425 theme -> one per-stock-quotes read.
live-smoke-t1537:
	$(call run_smoke,market_session_price::live_smoke_t1537)

## t1452 (거래량상위) smoke: token -> one single-page top-volume read.
live-smoke-t1452:
	$(call run_smoke,paginated_boards::live_smoke_t1452)

## Remaining single-page paginated rank/screen smokes (one post_paginated each).
live-smoke-t1403:
	$(call run_smoke,paginated_boards::live_smoke_t1403)
live-smoke-t1441:
	$(call run_smoke,paginated_boards::live_smoke_t1441)
live-smoke-t1463:
	$(call run_smoke,paginated_boards::live_smoke_t1463)
live-smoke-t1466:
	$(call run_smoke,paginated_boards::live_smoke_t1466)
live-smoke-t1489:
	$(call run_smoke,paginated_boards::live_smoke_t1489)
live-smoke-t1492:
	$(call run_smoke,paginated_boards::live_smoke_t1492)
...
live-smoke-t1481:
	$(call run_smoke,paginated_boards::live_smoke_t1481)
...
live-smoke-t1482:
	$(call run_smoke,paginated_boards::live_smoke_t1482)
...
live-smoke-t1866:
	$(call run_smoke,paginated_boards::live_smoke_t1866)
...
live-smoke-t1859:
	$(call run_smoke,market_session_flow::live_smoke_t1859)
...
live-smoke-t1826:
	$(call run_smoke,market_session_flow::live_smoke_t1826)
...
live-smoke-t1825:
	$(call run_smoke,market_session_flow::live_smoke_t1825)

## Wave 1 ELW universe/list reads (no caller input; non-empty success -> flip).
live-smoke-t9905:
	$(call run_smoke,market_session_masters::live_smoke_t9905)
live-smoke-t9907:
	$(call run_smoke,market_session_masters::live_smoke_t9907)
live-smoke-t8431:
	$(call run_smoke,market_session_masters::live_smoke_t8431)
live-smoke-t8430:
	$(call run_smoke,market_session_masters::live_smoke_t8430)
...
live-smoke-t1901:
	$(call run_smoke,market_session_charts::live_smoke_t1901)
...
live-smoke-t1906:
	$(call run_smoke,market_session_charts::live_smoke_t1906)
...
live-smoke-t8450:
	$(call run_smoke,market_session_price::live_smoke_t8450)
...
live-smoke-t1638:
	$(call run_smoke,market_session_flow::live_smoke_t1638)
...
live-smoke-t1308:
	$(call run_smoke,market_session_charts::live_smoke_t1308)
...
live-smoke-t1449:
	$(call run_smoke,market_session_charts::live_smoke_t1449)
...
live-smoke-t1621:
	$(call run_smoke,market_session_charts::live_smoke_t1621)
...
live-smoke-t2545:
	$(call run_smoke,market_session_charts::live_smoke_t2545)
...
live-smoke-t8406:
	$(call run_smoke,market_session_charts::live_smoke_t8406)
...
live-smoke-t8407:
	$(call run_smoke,market_session_price::live_smoke_t8407)
...
live-smoke-t1631:
	$(call run_smoke,market_session_flow::live_smoke_t1631)
...
live-smoke-t1632:
	$(call run_smoke,market_session_flow::live_smoke_t1632)
...
live-smoke-t1633:
	$(call run_smoke,market_session_flow::live_smoke_t1633)
...
live-smoke-t1904:
	$(call run_smoke,market_session_charts::live_smoke_t1904)
...
live-smoke-t1927:
	$(call run_smoke,market_session_flow::live_smoke_t1927)
...
live-smoke-t1941:
	$(call run_smoke,market_session_flow::live_smoke_t1941)
...
live-smoke-t1702:
	$(call run_smoke,market_session_flow::live_smoke_t1702)
...
live-smoke-t1717:
	$(call run_smoke,market_session_flow::live_smoke_t1717)
...
live-smoke-t1475:
	$(call run_smoke,market_session_flow::live_smoke_t1475)
## defaults to "" = the full LP-target list; reachable under closure).
## Override with LS_LIVE_SMOKE_T1959_SHCODE.
live-smoke-t1959:
	$(call run_smoke,market_session_charts::live_smoke_t1959)
...
live-smoke-t1950:
	$(call run_smoke,market_session_masters::live_smoke_t1950)
...
live-smoke-t1954:
	$(call run_smoke,market_session_masters::live_smoke_t1954)
...
live-smoke-t1971:
	$(call run_smoke,market_session_masters::live_smoke_t1971)
...
live-smoke-t1972:
	$(call run_smoke,market_session_masters::live_smoke_t1972)
...
live-smoke-t1974:
	$(call run_smoke,market_session_masters::live_smoke_t1974)
...
live-smoke-t1956:
	$(call run_smoke,market_session_masters::live_smoke_t1956)
...
live-smoke-t1310:
	$(call run_smoke,paginated_misc::live_smoke_t1310)
live-smoke-t1404:
	$(call run_smoke,paginated_misc::live_smoke_t1404)
...
live-smoke-t1410:
	$(call run_smoke,paginated_misc::live_smoke_t1410)
...
## empty 00707 -> PENDING.
live-smoke-t1411:
	$(call run_smoke,paginated_boards::live_smoke_t1411)
...
live-smoke-t1488:
	$(call run_smoke,paginated_boards::live_smoke_t1488)
...
live-smoke-t1636:
	$(call run_smoke,paginated_boards::live_smoke_t1636)
...
live-smoke-t1809:
	$(call run_smoke,paginated_misc::live_smoke_t1809)
...
live-smoke-t1109:
	$(call run_smoke,paginated_misc::live_smoke_t1109)
...
live-smoke-t1301:
	$(call run_smoke,paginated_misc::live_smoke_t1301)
...
live-smoke-t1486:
	$(call run_smoke,paginated_misc::live_smoke_t1486)
...
live-smoke-t8454:
	$(call run_smoke,paginated_misc::live_smoke_t8454)
...
live-smoke-t1637:
	$(call run_smoke,paginated_misc::live_smoke_t1637)
...
## Non-empty -> flip; empty -> PENDING.
live-smoke-t1602:
	$(call run_smoke,paginated_misc::live_smoke_t1602)
...
## Non-empty -> flip; empty -> PENDING.
live-smoke-t1603:
	$(call run_smoke,paginated_misc::live_smoke_t1603)
...
live-smoke-t1617:
	$(call run_smoke,paginated_misc::live_smoke_t1617)
...
## Non-empty -> flip; empty -> PENDING.
live-smoke-t1752:
	$(call run_smoke,paginated_misc::live_smoke_t1752)
...
live-smoke-t1771:
	$(call run_smoke,paginated_misc::live_smoke_t1771)
live-smoke-t9942:
	$(call run_smoke,market_session_masters::live_smoke_t9942)
...
live-smoke-t1958:
	$(call run_smoke,market_session_masters::live_smoke_t1958)
live-smoke-t1964:
	$(call run_smoke,market_session_masters::live_smoke_t1964)

## Wave 2 market-flow analytics reads (documented gubun defaults; non-empty -> flip).
live-smoke-t1601:
	$(call run_smoke,market_session_flow::live_smoke_t1601)
live-smoke-t1615:
	$(call run_smoke,market_session_flow::live_smoke_t1615)
live-smoke-t1640:
	$(call run_smoke,market_session_flow::live_smoke_t1640)
live-smoke-t1662:
	$(call run_smoke,market_session_flow::live_smoke_t1662)
live-smoke-t1664:
	$(call run_smoke,market_session_flow::live_smoke_t1664)
...
live-smoke-t8455:
	$(call run_smoke,market_session_masters::live_smoke_t8455)
...
live-smoke-t8460:
	$(call run_smoke,market_session_masters::live_smoke_t8460)

## t8463 (KRX야간파생 투자자시간대별) smoke: token -> one investor-by-timeslot read (N/F/101).
## venue_session krx_extended (~18:00-05:00 KST).
live-smoke-t8463:
	$(call run_smoke,market_session_flow::live_smoke_t8463)
...
live-smoke-g3101:
	$(call run_smoke,market_session_price::live_smoke_g3101)

live-smoke-g3104:
	$(call run_smoke,market_session_masters::live_smoke_g3104)

live-smoke-g3106:
	$(call run_smoke,market_session_price::live_smoke_g3106)

live-smoke-g3102:
	$(call run_smoke,market_session_charts::live_smoke_g3102)

live-smoke-g3103:
	$(call run_smoke,market_session_charts::live_smoke_g3103)

## g3190 (해외주식 마스터): token -> one master-list read (US, exchange 2, 10 rows).
live-smoke-g3190:
	$(call run_smoke,market_session_masters::live_smoke_g3190)

## o3101 (해외선물 마스터): token -> one futures-master read (gubun=all).
live-smoke-o3101:
	$(call run_smoke,market_session_masters::live_smoke_o3101)

## o3121 (해외선물옵션 마스터): token -> one option-master read (MktGb=O).
live-smoke-o3121:
	$(call run_smoke,market_session_masters::live_smoke_o3121)

## o3105 (해외선물 현재가): token -> one futures-quote read (CUSN23).
live-smoke-o3105:
	$(call run_smoke,market_session_price::live_smoke_o3105)

## o3106 (해외선물 현재가호가): token -> one futures-order-book read (ADM23).
live-smoke-o3106:
	$(call run_smoke,market_session_price::live_smoke_o3106)

## o3125 (해외선물옵션 현재가): token -> one option-quote read (F/HSIM23).
live-smoke-o3125:
	$(call run_smoke,market_session_price::live_smoke_o3125)

## o3126 (해외선물옵션 현재가호가): token -> one option-order-book read (F/ADM23).
live-smoke-o3126:
	$(call run_smoke,market_session_price::live_smoke_o3126)

## Domestic stock master/reference breadth wave (plan -004).
live-smoke-t9945:
	$(call run_smoke,market_session_masters::live_smoke_t9945)
live-smoke-t3202:
	$(call run_smoke,market_session_charts::live_smoke_t3202)
live-smoke-t3401:
	$(call run_smoke,paginated_misc::live_smoke_t3401)
live-smoke-t3518:
	$(call run_smoke,paginated_misc::live_smoke_t3518)
live-smoke-t3521:
	$(call run_smoke,market_session_flow::live_smoke_t3521)
live-smoke-o3103:
	$(call run_smoke,paginated_boards::live_smoke_o3103)
live-smoke-o3104:
	$(call run_smoke,market_session_charts::live_smoke_o3104)
live-smoke-o3108:
	$(call run_smoke,paginated_boards::live_smoke_o3108)
live-smoke-o3116:
	$(call run_smoke,paginated_boards::live_smoke_o3116)
live-smoke-o3117:
	$(call run_smoke,paginated_boards::live_smoke_o3117)
live-smoke-o3123:
	$(call run_smoke,paginated_boards::live_smoke_o3123)
live-smoke-o3127:
	$(call run_smoke,market_session_price::live_smoke_o3127)
## F-O open-window flip wave (plan -001): contract self-sourced from t8467.
live-smoke-t8427:
	$(call run_smoke,market_session_charts::live_smoke_t8427)
live-smoke-t2210:
	$(call run_smoke,market_session_charts::live_smoke_t2210)
live-smoke-t2424:
	$(call run_smoke,market_session_charts::live_smoke_t2424)
live-smoke-t2541:
	$(call run_smoke,paginated_misc::live_smoke_t2541)
live-smoke-t2214:
	$(call run_smoke,paginated_misc::live_smoke_t2214)
## t8428 deposit-balance trend — domestic, default (MAIN) lane.
live-smoke-t8428:
	$(call run_smoke,market_session_charts::live_smoke_t8428)
live-smoke-o3128:
	$(call run_smoke,paginated_boards::live_smoke_o3128)
live-smoke-o3136:
	$(call run_smoke,paginated_boards::live_smoke_o3136)
live-smoke-o3137:
	$(call run_smoke,paginated_boards::live_smoke_o3137)
live-smoke-o3139:
	$(call run_smoke,paginated_boards::live_smoke_o3139)
live-smoke-t8462:
	$(call run_smoke,market_session_flow::live_smoke_t8462)
live-smoke-t8410:
	$(call run_smoke,paginated_boards::live_smoke_t8410)
live-smoke-t8451:
	$(call run_smoke,paginated_boards::live_smoke_t8451)
live-smoke-t8419:
	$(call run_smoke,paginated_boards::live_smoke_t8419)
live-smoke-t4203:
	$(call run_smoke,paginated_boards::live_smoke_t4203)

live-smoke-t8417:
	$(call run_smoke,paginated_boards::live_smoke_t8417)

live-smoke-t8418:
	$(call run_smoke,paginated_boards::live_smoke_t8418)

live-smoke-t8411:
	$(call run_smoke,paginated_boards::live_smoke_t8411)

live-smoke-t8452:
	$(call run_smoke,paginated_boards::live_smoke_t8452)

live-smoke-t8453:
	$(call run_smoke,paginated_boards::live_smoke_t8453)

live-smoke-t1302:
	$(call run_smoke,market_session_charts::live_smoke_t1302)

live-smoke-t8464:
	$(call run_smoke,paginated_boards::live_smoke_t8464)

live-smoke-t8465:
	$(call run_smoke,paginated_boards::live_smoke_t8465)

live-smoke-t8466:
	$(call run_smoke,paginated_boards::live_smoke_t8466)

live-smoke-t2216:
	$(call run_smoke,market_session_charts::live_smoke_t2216)

live-smoke-t8405:
	$(call run_smoke,paginated_boards::live_smoke_t8405)

live-smoke-t1444:
	$(call run_smoke,paginated_boards::live_smoke_t1444)

live-smoke-t1422:
	$(call run_smoke,paginated_boards::live_smoke_t1422)

live-smoke-t1427:
	$(call run_smoke,paginated_boards::live_smoke_t1427)

live-smoke-t1442:
	$(call run_smoke,paginated_boards::live_smoke_t1442)

live-smoke-t1405:
	$(call run_smoke,paginated_boards::live_smoke_t1405)

live-smoke-t1960:
	$(call run_smoke,paginated_boards::live_smoke_t1960)

live-smoke-t1961:
	$(call run_smoke,paginated_boards::live_smoke_t1961)

live-smoke-t1966:
	$(call run_smoke,paginated_boards::live_smoke_t1966)

live-smoke-t1921:
	$(call run_smoke,paginated_boards::live_smoke_t1921)

live-smoke-t1532:
	$(call run_smoke,market_session_masters::live_smoke_t1532)

live-smoke-t1533:
	$(call run_smoke,market_session_masters::live_smoke_t1533)

live-smoke-t1926:
	$(call run_smoke,market_session_flow::live_smoke_t1926)

live-smoke-t1764:
	$(call run_smoke,market_session_charts::live_smoke_t1764)

live-smoke-t1903:
	$(call run_smoke,market_session_charts::live_smoke_t1903)
...
$(eval $(call NEG_PROBE_TARGET,t1444,live_smoke_t1444_negative))

.PHONY: live-smoke-t1101-negative live-smoke-t1102-negative live-smoke-cspaq12200-negative live-smoke-t0425-negative live-smoke-token-negative live-smoke-cspat00601-negative live-smoke-cspat00701-negative live-smoke-cspat00801-negative live-smoke-t8410-negative live-smoke-t8430-negative live-smoke-t1444-negative
